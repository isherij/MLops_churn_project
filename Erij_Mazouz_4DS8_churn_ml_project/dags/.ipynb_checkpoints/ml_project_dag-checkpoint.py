...
def check_formatting():
    subprocess.run(["black", "--check", "."], check=True)  # Added check=True for proper error handling
...
def run_tests():
    subprocess.run(["pytest", "tests/"], check=True)  # Added check=True for proper error handling
...
def cleanup_task():
    ...
    if os.path.exists(model_file):  # Ensure the model file exists before trying to remove it
